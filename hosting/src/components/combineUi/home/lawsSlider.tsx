...
const LawsSlider: FC<Props> = ({ lawArray }): JSX.Element => {
  return (
    <Splide
    ...
    >
      {lawArray.map((item, _i) => {
        return (
          <SplideSlide key={_i} className="h-full w-full">
            <CardContent className="flex-1">
              <div className="flex justify-between flex-col">
                <div className="flex lg:items-center gap-2 flex-wrap lg:flex-row flex-col">
                  <div className="relative flex items-center justify-center self-start lg:bottom-0 -bottom-2">
                    <PiCrownSimple size={30} className="text-yellow-400 -rotate-6" />
                    <div className="absolute text-xs">{_i + 1}</div>
                  </div>
                  <p className="">関税定率法（明治四十三年法律第五十四号）</p>
                </div>
                <div className="flex gap-2 mt-2">
                  <Badge variant="outline">刑事</Badge>
                  <Badge variant="outline">法律</Badge>
                </div>
              </div>
              <Button className="absolute right-0 m-4 h-10 mb-6" asChild>
