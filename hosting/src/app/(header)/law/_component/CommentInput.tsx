...
import { useCreateLawCommentMutation } from "@/graphql/type";
...
const CommentInput = ({ law_id }: { law_id: string }) => {
  const { register, getValues, setValue, reset, formState } = useForm<Schema>();

  const [mutate] = useCreateLawCommentMutation({ refetchQueries: ["findLawComments"] });
    ...
    <div className="flex flex-1">
      <div className="flex flex-1 justify-between gap-2 pr-2">
        <TextArea<Schema>
          ...
          width="h-10 w-full"
          onSubmit={onSubmit}
          Icon={<VscSend size={22} className={`relative top-2 transition-all active:opacity-25`} />}
